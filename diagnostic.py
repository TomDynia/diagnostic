...
# 2. CHARTE GRAPHIQUE DYNIA (CSS PERSONNALISÉ)
st.markdown("""
    <style>
    :root {
        --brand-deep: #0c2461;
    ...
    /* Personnalisation du slider (Jauge) */
...
# 4. CONFIGURATION DES QUESTIONS (LOGIQUE SÉCURISÉE)
# On utilise un dictionnaire pour mapper les textes aux scores réels
QUESTIONS_DATA = [
    {
        "id": "emails",
        ...
        "scores": {"Tout manuel (0)": 0, "Quelques modèles (3)": 3, "IA génère des brouillons (7)": 7, "Entièrement automatisé (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Saisie manuelle Word/Excel (0)": 0, "Logiciel de facturation simple (4)": 4, "Génération auto via CRM (8)": 8, "IA analyse & génère le devis (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Jamais (0)": 0, "Rarement (3)": 3, "Hebdomadaire (6)": 6, "Quotidien (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Silos isolés (0)": 0, "Quelques imports manuels (3)": 3, "Quelques automatisations (7)": 7, "Écosystème fluide (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Inexistante / Papier (0)": 0, "Fichier Excel (4)": 4, "CRM peu utilisé (7)": 7, "CRM à jour & synchronisé (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Notes manuscrites (0)": 0, "Notes tapées manuellement (4)": 4, "Enregistrement simple (7)": 7, "Synthèse IA automatique (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Page blanche (0)": 0, "Inspiration manuelle (4)": 4, "IA aide à la rédaction (8)": 8, "Flux de contenu automatisé (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Recherche manuelle (0)": 0, "Achat de listes (4)": 4, "Scraping manuel (7)": 7, "Prospection IA automatisée (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Réponse manuelle systématique (0)": 0, "FAQ statique (4)": 4, "Quelques macros (7)": 7, "Base de connaissance IA (10)": 10},
    ...
    },
    {
        ...
        "scores": {"Mémoire humaine uniquement (0)": 0, "Documents éparpillés (4)": 4, "Wiki structuré (8)": 8, "Wiki IA consultable (10)": 10},
...
with st.form("audit_form"):
    st.subheader("Diagnostic de maturité")
    user_responses = {}
    
    for item in QUESTIONS_DATA:
        user_responses[item["id"]] = st.select_slider(
            item["q"], 
...
if submitted:
    ...
    for item in QUESTIONS_DATA:
        selected_val = user_responses[item["id"]]
        score = item["scores"][selected_val]
        total_score += score
        analysis_list.append({"advice": item["advice"], "score": score})
    
    # Tri pour trouver les 3 chantiers prioritaires (scores les plus bas)
    analysis_list.sort(key=lambda x: x["score"])
    top_3_priorities = analysis_list[:3]

    # Détermination du profil (Nuances de Bleu Dynia)
    if total_score <= 40:
        profile, focus, color = "Débutant IA", "Installation des outils de base (Le Cockpit)", "#4a69bd"
    elif total_score <= 70:
        profile, focus, color = "Intermédiaire", "Connexion des outils & Workflows (n8n)", "#1e3799"
    else:
        profile, focus, color = "Avancé", "Optimisation stratégique (Claude Code)", "#0c2461"

    # Affichage du rapport
    st.markdown('<div class="result-card">', unsafe_allow_html=True)
    st.markdown("<h2 style='text-align:center;'>Votre Score de Maturité</h2>", unsafe_allow_html=True)
    st.markdown(f'<div class="score-display">{total_score}/100</div>', unsafe_allow_html=True)
    st.markdown(f"""
        <div style='text-align:center; margin-bottom:2rem;'>
            <span style='background-color:{color}; color:white; padding:10px 30px; border-radius:30px; font-weight:bold; font-size:1.2rem;'>
                Profil : {profile}
            </span>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.subheader("Le Conseil de Thomas : Vos 3 chantiers prioritaires")
    for priority in top_3_priorities:
        st.markdown(f"""
    ...
    # 7. CONVERSION (CTA UNIQUE BLEU DYNIA)
    st.markdown("<br>", unsafe_allow_html=True)
    st.link_button(
        "Réserver ma session de coaching pour implanter ces solutions", 
...
st.markdown("""
    <div style="text-align: center; color: #94a3b8; font-size: 0.8rem;">
        © 2024 DYNIA IA - Diagnostic de Performance Opérationnelle
    </div>
""", unsafe_allow_html=True)