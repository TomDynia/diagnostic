...
# 3. EN-TÊTE (LOGO LOCAL ET TITRE SANS EMOJI)
...
if submitted:
    ...
    # Profils basés sur le Bleu Dynia (dégradés de bleu)
    if total_score <= 40:
    ...
    st.markdown(f"<div style='text-align:center; margin-bottom:2rem;'><span style='background-color:{color}; color:white; padding:10px 30px; border-radius:30px; font-weight:bold; font-size:1.2rem;'>Profil : {profile}</span></div>", unsafe_allow_html=True)
    ...
    # 7. CONVERSION (BOUTON UNIQUE BLEU DYNIA)
    st.markdown("<br>", unsafe_allow_html=True)
    st.link_button("Réserver ma session de coaching pour implanter ces solutions", "mailto:thomas@dynia.fr", type="primary", use_container_width=True)
